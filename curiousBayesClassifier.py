...
class CuriousBayesClassifier(BayesClassifier):
    ...
    def __init__(self, data, columnsInfo):
        ...
        self.indecies_of_data_rows = list(range(len(self.data)))
        self.learning_order = []
    ...
    def calculate_step_without_update(self, row):
        ...
        for t in range(2):
            ...
            new_p[t] = eval("self.theta"+str(n[:-1]+[t])) * p
    ...
    def fit(self, numCandidatesInIter):
        iter = 1
        for _ in tqdm(range(len(self.data))):
            candidates = self.generateCurrentRoundCandidates(
                numCandidatesInIter)
            ...
            exec("self.theta" +
                 str(list(chosenRow[:-1])) + "= best_IG_cand_p[2]")
    ...
    def generateCurrentRoundCandidates(self, numCandidates):
        if(len(self.indecies_of_data_rows) >= numCandidates):
            current_round_candidates = np.random.choice(
                self.indecies_of_data_rows, numCandidates, replace=False)
        else:
            current_round_candidates = np.random.choice(
                self.indecies_of_data_rows, len(self.indecies_of_data_rows), replace=False)
